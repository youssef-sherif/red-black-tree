			...
			while(u.isDoubleBlack() && u.getParent() != null) {
				...
				if(sibling.isBlack()) {
					...
					if(sibling.getLeftChild().isRed()) {
						sChild1 = sibling.getLeftChild();
					} else if(sibling.getRightChild().isRed()) {
						sChild2 = sibling.getRightChild();
					}
					
					//Left Left Case
					if(sibling.getParent().getLeftChild() == sibling && sChild1.isRed() && sChild2.isRed()) {
